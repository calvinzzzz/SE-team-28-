...
{
 ((!list_empty_(h, "./ccan/list/list.h" ":" "328")) ? (void) (0) : __assert_fail ("!list_empty_(h, \"./ccan/list/list.h\" \":\" \"328\")", "./ccan/list/list.h", 328, __PRETTY_FUNCTION__));
...
vm_opt_regexpmatch1(VALUE recv, VALUE obj)
{
 ...
 return __extension__({ const int rb_funcall_argc = (1); const VALUE rb_funcall_args[] = {obj}; const int rb_funcall_nargs = (int)(sizeof(rb_funcall_args) / sizeof(VALUE)); rb_funcallv(recv, idEqTilde, (((rb_funcall_argc) <= (rb_funcall_nargs)) ? (rb_funcall_argc) : (rb_fatal("argc(%d) exceeds actual arguments(%d)", rb_funcall_argc, rb_funcall_nargs), 0)), rb_funcall_nargs ? rb_funcall_args : ((void *)0)); });
    }
}
static VALUE
...
#define RUBY_SAFE_LEVEL_INVALID_P(level) __extension__(__builtin_constant_p(level) && ((level) < 0 || RUBY_SAFE_LEVEL_MAX < (level)))
#define RUBY_SAFE_LEVEL_CHECK(level,type) (RUBY_SAFE_LEVEL_INVALID_P(level) ? ruby_safe_level_2_ ##type() : (level))
...
#define RB_OBJ_WB_UNPROTECT_FOR(type,obj) (RGENGC_WB_PROTECTED_ ##type ? OBJ_WB_UNPROTECT((VALUE)(obj)) : ((VALUE)(obj)))
